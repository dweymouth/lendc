...
let print_portfolios portfolios =
  print_endline ((List.length portfolios |> string_of_int) ^ " total portfolios");
  List.iter (fun (portfolio : portfolio) ->
    printf "%d %s %s\n" portfolio.id portfolio.name
      (match portfolio.description with None -> "" | Some s -> s)) portfolios
